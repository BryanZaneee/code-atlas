...
import { REPO_ROOT, FIXTURE_DIR } from "./helpers.mjs";

const ATLAS = path.join(REPO_ROOT, "bin", "atlas.mjs");

/** A throwaway copy of a fixture — init writes, so it never touches fixtures/. */
function copyFixture(name) {
  const dir = mkdtempSync(path.join(os.tmpdir(), "atlas-init-"));
...
test("init writes a config the loader accepts, naming what it detected", async () => {
...
test("init refuses to overwrite a config that already exists", () => {
  const dir = copyFixture("flat-app");
  const file = path.join(dir, "atlas.config.mjs");
  try {
    atlas(["init", "--repo", dir]);
...
test("a repository built with its own generated config still renders", () => {
  const dir = copyFixture("flat-app");
  try {
    atlas(["init", "--repo", dir]);
    const out = atlas([
