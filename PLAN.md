...
Highlighting is **Prism 1.29.0, vendored** — committed into `src/viewer/` as a file, not installed as a dependency, so `atlas build` still emits one self-contained HTML and running the tool against someone else's repo still needs no install. ~27 KB, ships ts/js/jsx/tsx, python, sql, json.
