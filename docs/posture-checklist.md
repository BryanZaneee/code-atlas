...
13. **A dependency is a conversation, not a rule to obey.** Propose one in the
    ...
    **Vendoring is the third option** and has precedent (Prism). A vendored
    file is still a dependency, tested like anything else in `src/`, with its
    version and licence in its header.
